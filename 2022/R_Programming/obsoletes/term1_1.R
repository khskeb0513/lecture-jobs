...
제300조(공작물의 공동사용) ①승역지의 소유자는 지역권의 행사를 방해하지 아니하는 범위 내에서 지역권자가 지역권의 행사를 위하여 승역지에 설치한 공작물을 사용할 수 있다.
...
제682조(복임권의 제한) ①수임인은 위임인의 승낙이나 부득이한 사유없이 제삼자로 하여금 자기에 갈음하여 위임사무를 처리하게 하지 못한다. <개정 2014. 12. 30.>
...
제942조(후견인의 채권ㆍ채무의 제시) ① 후견인과 피후견인 사이에 채권ㆍ채무의 관계가 있고 후견감독인이 있는 경우에는 후견인은 재산목록의 작성을 완료하기 전에 그 내용을 후견감독인에게 제시하여야 한다.
...
제959조의20(후견계약과 성년후견ㆍ한정후견ㆍ특정후견의 관계) ① 후견계약이 등기되어 있는 경우에는 가정법원은 본인의 이익을 위하여 특별히 필요할 때에만 임의후견인 또는 임의후견감독인의 청구에 의하여 성년후견, 한정후견 또는 특정후견의 심판을 할 수 있다. 이 경우 후견계약은 본인이 성년후견 또는 한정후견 개시의 심판을 받은 때 종료된다.
...
di1_str = str_split(di1_str, '\n')[[1]]
di2_str = str_split(di2_str, '\n')[[1]]
di3_str = str_split(di3_str, '\n')[[1]]
di4_str = str_split(di4_str, '\n')[[1]]
di5_str = str_split(di5_str, '\n')[[1]]

minbeop_tbl = tibble(document = '제1편 총칙', text = di1_str)
minbeop_tbl = rbind(minbeop_tbl, tibble(document = '제2편 물권', text = di2_str))
minbeop_tbl = rbind(minbeop_tbl, tibble(document = '제3편 채권', text = di3_str))
minbeop_tbl = rbind(minbeop_tbl, tibble(document = '제4편 친족', text = di4_str))
minbeop_tbl = rbind(minbeop_tbl, tibble(document = '제5편 상속', text = di5_str))

library(tidytext)
library(RcppMeCab)

minbeop_token_tbl = minbeop_tbl %>%
  tidytext::unnest_tokens(word, text, token = posParallel) %>%
  filter(str_detect(word, 'nn|vv'))

minbeop_token_tbl$word = sapply(str_split(minbeop_token_tbl$word, '/'), "[[", 1)
...
minbeop_token_tbl = minbeop_tbl %>%
  tidytext::unnest_tokens(word, text, token = posParallel, drop = F) %>%
  filter(str_detect(word, 'nn|vv'))

minbeop_token_tbl$word = sapply(str_split(minbeop_token_tbl$word, '/'), "[[", 1)

minbeop_token_tbl = minbeop_token_tbl %>%
