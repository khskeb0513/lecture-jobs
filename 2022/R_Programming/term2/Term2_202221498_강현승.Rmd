...
이 실험에서 아래와 같은 가정이 맞는지 확인해보았다.

#### 가정 1

과거보다 지금 기업들의 공시자료에서 CSR 키워드가 더 많이 사용되고 있다.

#### 가정 2

기업들이 공시자료에서 CSR 키워드를 더 많이 사용할수록 외국인 투자자가 증가한다.

#### 가정 3

기업들의 공시자료에서 CSR 키워드를 많이 사용하였을 때 기업가치가 오를 것이다.

## 선정이유

최근 친환경 등을 내세우는 ESG가 이른바 큰손들인 각국 연기금의 투자 판단의 지표로 삼는다는 뉴스를 접하였다.

https://www.hankyung.com/economy/article/202108181875i

<img src="https://git.ajou.ac.kr/h5k/ebiz-r-programming/raw/main/term2/news_1.png">

상장사들은 공시자료를 전자공시시스템에 공시할 의무가 있고, 사업보고서에는 회사들의 현재 사업에 대한 상세한 설명이 보고되고 있다. 따라서 세계적인 추세인 ESG 가 공시자료 상에서도 확인 가능한지, 그리고 이러한 ESG 키워드의 사용이 기업가치와 외국인 투자자들에게 흥미를 주는지 궁금하였다.

실제로 최근에 ESG 공시 의무화가 목전이라는 소식이 들려왔다. 따라서 보고서상에서도 ESG 키워드의 사용 비중 증가를 충분히 관측 가능할 것이라고 예상하여 주제를 선정하게 되었다.

```
자본시장을 중심으로 ESG 공시 의무화에 대한 논의가 뜨겁다. 전혀 움직일 것 같지 않던 미국이 지난 3월, IFRS 산하 국제지속가능성기준위원회(ISSB)가 4월, 이미 ESG 공시의무화를 시행하고 있는 EU가 한층 광범위한 내용을 담은 기준을 3월 제시했다. 그리고 글로벌 3대 주요 ESG 공시 기준 모두 이해관계자 의견수렴을 거쳐 최종(안) 도출을 위한 마무리 작업이 한창이다. 올해 말부터 늦어도 2023년 1분기 중 ESG 의무 공시(안) 공개가 예상된다.

출처: https://news.mt.co.kr/mtview.php?no=2022110318245371708
```

이번 보고서에서는 ESG 키워드를 직접 CEO 인사말 등에서 추출하는 대신, 기존 논문을 참고하여 ESG 키워드로 사용하였다.

윤지혜, 이종화. (2021). 토픽모델링을 활용한 CSR 키워드 트렌드 분석. 인터넷전자상거래연구, 21(5), 73-91.

<img src="https://git.ajou.ac.kr/h5k/ebiz-r-programming/raw/main/term2/csrs.png">
...
csr 키워드 포함 어절 수 데이터와 보고서목록 데이터를 합하여 `가정1`을 위한 데이터가 완성되었다.

데이터의 분포를 확인하기 위해 x축이 `보고년도`, y축이 `csr 키워드 포함 어절 수`로된 꺾은선 그래프를 생성하는 시각화를 진행하였다.
...
그러나 간혹 급격하게 `csr 키워드 사용 어절 수`가 줄어든 보고서도 눈에 띈다.<br>
왜 이런 데이터를 갖게 되었는지 알아야 할 필요가 있다.<br>
따라서 이 보고서가 어느 회사의 몇년도 보고서인지 알기 위해 위의 그래프와 같은 데이터로 y축의 크기를 10,000 이하로 제한하여 그래프를 생성했다.<br>
또한 색상이 많아 회사인지 분간하기 어려우므로 반응형 그래프로 생성하였다.
...
그래프를 통해 큰 하락폭을 보여준 보고서는 다음과 같다.
...
아래 코드들을 통해 두 보고서의 보고서 등록연번을 알아낼 수 있다.
...
첫째로, 2019년 케이티앤지의 하락은 잘못된 데이터를 불러왔기 때문이다.<br>
2019년에 "사업보고서" 단어를 포함한 서류가 2 건 제출되었는데, 이중 하나가 사업보고서제출기한연장신고서였다.<br>
따라서 잘못 불러와진 보고서의 n 값을 결측치`NA`처리하였다.

둘째로, 2017년 신한지주의 하락은 정상이다.<br>
해당 보고서를 열람해보았을 때 정상적인 보고서였다.<br>
따라서 계산에 포함해야 한다.

아래 코드를 통해 결측치 처리하였고, 데이터를 바탕으로 `n`변량을 정규화하여 그래프를 생성하였다.
...
위 그래프는 년도가 지날수록 CSR 키워드가 많이 사용됨을 보여준다.<br>
그러나 빈도 변량을 그대로 사용한 그래프는 아래와 같은 의문점을 시사한다.

- 공시자료의 전체 단어 수가 증가한 것 아닌가?

따라서 빈도 변량 대신 전체 문서 단어에서 빈도를 나눈 `n_weight` 변량을 정규화하여 시각화를 진행하였다.
...
그래프에서 뚜렷한 상관관계를 보여주지 않고 있다.

두 그래프는 정반대의 결과를 보여준다.<br>
따라서 두 그래프의 결과를 정량적으로 확인할 수 있도록 각 그래프에서 사용된 두 변량을 상관계수로 계산하여보았다.
...
`년도`와 `CSR 키워드 포함 어절의 보고서 내 사용 빈도`는 양의 상관계수를 가지므로 상관관계가 없지 않다.
`년도`와 `CSR 키워드 포함 어절의 보고서 내 비중`은 음의 상관계수를 가지므로 상관관계가 있다고 할 수 없다.

따라서 과거보다 지금 기업들의 공시자료에서 CSR 키워드가 더 많은 빈도로 출현하지만,<br>
더 큰 비중을 차지한다고 볼 수 없다.

## 가정 2 확인

[가정 2]과거보다 지금 기업들이 공시자료에서 CSR 키워드를 더 많이 사용함에 따라 외국인 투자자가 증가하였을 것이다.

아래 코드를 통해 네이버 증권 페이지를 크롤링하여 종목별 외국인 투자자 보유율을 가져올 수 있다.
...
종목별 외국인 투자자 보유율 데이터를 사용하기 전에 이 데이터에 이상값이 있는지 확인하고자 한다.<br>
...
육안으로 확인하였을 때, 10,000% 이상의 지분을 외국인 투자자가 가진 경우를 발견하였다.<br>
아래 코드를 통해 해당 행을 확인하여 본다.

```{r}
filtered_stock_data %>% filter(forign_own_percent > 10000)
```

위 3개 행들은 모두 신주상장이 원인인 것으로 밝혀졌다.<br>
아래 그림은 그 예인데, 첫 번째 행인 종목코드003670 (주)포스코케미칼의 2019년 5월 28일 공시자료이다.<br><br>
<img src="https://git.ajou.ac.kr/h5k/ebiz-r-programming/raw/main/term2/img1.jpg"><br><br>
이는 가정2를 규명하는데 관계가 없으므로 데이터 행을 삭제하였다.

다시 분포 확인을 위해 시각화를 진행하였다.<br>
행의 삭제 후에도 지분율이 100% 이상인 경우가 보고된다.
...
아래 선별된 이상값 행들은 액면분할로 인해 지분율이 100% 이상이 되었다고 규명되었다.<br>
이또한 가정2를 규명하는데 관계가 없으므로 데이터 행을 삭제하였다.<br>
따라서 아래 코드를 통해서 행들을 데이터에서 제외하였다.
...
이상값 문제들을 해결하였으므로 외국인 투자자 지분율 데이터의 평균을 정규화하여 시각화를 진행하였다.
...
그래프에서 뚜렷한 상관관계를 보여주지 않는다.

따라서 준비된 데이터에서 상관관계를 가질 것으로 예상했던

- `csr 키워드 포함 어절 수`와 `종목별 연간 외국인 투자자 지분율 평균`
- `csr 키워드 포함 어절 출현빈도`와 `종목별 연간 외국인 투자자 지분율 평균`
...
`csr 키워드 포함 어절 수`와 `종목별 연간 외국인 투자자 지분율 평균`는 양의 상관계수(0.2743097)를 가지므로 상관관계가 없지 않다.
`csr 키워드 포함 어절 비중`과 `종목별 연간 외국인 투자자 지분율 평균`는 양의 상관계수(0.05654015)를 보이나 너무 적어 유의미하지 않다.

따라서 종목별 연간 외국인 투자자 지분율이 csr 키워드 포함 어절 수에 영향을 미치거나 csr 키워드 포함 어절 수가 종목별 연간 외국인 투자자 지분율에 영향을 미치고 있다.

## 가정 3

[가정 3] 기업들의 공시자료에서 CSR 키워드를 많이 사용하였을 때 기업가치가 오를 것이다.

아래 코드들로 데이터를 준비한다.<br>
준비한 데이터가 정상적인지 분포를 확인하기 위해 시각화를 진행했다.
...
전체 기업을 그래프 상에 표시하는 경우 아예 분간이 불가능하기 때문에 head로 상위 행만 시각화를 진행하였다.<br>
그래프 상에 선들이 중구난방으로 보이는 듯 하지만 2020년에 완전히 폭삭 내려앉은 모습을 보아 올바른 현상임을 알 수 있다.

준비한 데이터를 바탕으로 시각화를 진행하였다.
...
낮은 CSR 키워드 비중을 가지는 보고서는 낮은 시가총액을 가지고 있음을 확인하였다.

이를 정량적으로 확인하기 위해 

- `csr 키워드 포함 어절 비중`와 `종목별 연간 시가총액 평균`
- `csr 키워드 포함 어절 출현빈도`와 `종목별 연간 시가총액 평균`

의 상관관계를 계산해보았다.

단 위의 그래프의 조건과 마찬가지로 각 파라미터의 양 극단을 데이터에서 제외하고 계산하였다.
...
- `csr 키워드 포함 어절 비중`와 `종목별 연간 시가총액 평균`는 양의 상관계수(0.2243652 )를 가지므로 상관관계가 없지 않다.
- `csr 키워드 포함 어절 출현빈도`와 `종목별 연간 시가총액 평균`는 양의 상관계수(0.1706868)를 가지므로 상관관계가 없지 않다.

따라서 사업보고서의 높은 CSR 키워드 비중은 기업가치를 증가시키거나, 높은 기업가치는 사업보고서의 높은 CSR을 이끌어낸다는 것을 확인하였다.

## 이 보고서 결론 정리

- 과거보다 지금 기업들의 공시자료에서 CSR 키워드가 더 많은 빈도로 출현하지만, 더 큰 비중을 차지한다고 볼 수 없다.
- 높은 외국인 투자자 지분율은 CSR 키워드 빈도를 증가시키거나, 높은 CSR 키워드 빈도는 외국인에게 투자를 유도한다고 볼 수 있다.
- 사업보고서의 높은 CSR 키워드 비중은 기업가치를 증가시키거나, 높은 기업가치는 사업보고서의 높은 CSR을 이끌어낸다는 것을 확인하였다.

### 한계점 

- 년도별 기업 가치 측정을 시가총액으로 하였는데, 이는 현가계산을 통해 더 정확하게 구해져야 한다.